...
<ul class="nav nav-pills nav-stacked col-md-2">
     <li class="active"><a href="servlet/UserServlet?action=show&authority=1">用户列表</a></li>
 	<li>
		<!-- href触发模态弹出窗元素 -->
		...
		<div id="mymodal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" id="mymodal-link">
    		<div class="modal-dialog">
				...
				<form action="servlet/UserServlet?action=add&authority=1" method="post">
 ...
 <caption>系统管理员账号管理</caption>
	...
	<tr>
		...
		<td>
			<a href="servlet/UserServlet?action=delete&authority=1&id=<%=userlist.get(i).getId()%>" onclick="delcfm()">删除</a>
		</td>
	</tr>
	<% 
		}
	...
	<%
	 ...
	 <nav>
	  <ul class="pagination">
	    <li <%=beforeclass%>>
	      <a <%=beforeclass%> href="servlet/UserServlet?action=show&authority=1<%=before %>" aria-label="Previous">
	    ...
	    %>
	    <li <%=active%>><a href="servlet/UserServlet?action=show&authority=1&pageNow=<%=i+1 %>"><%=i+1 %></a></li>
	    <% 
	    	}
	     %>
	    <li <%=nextclass%>>
	      <a <%=nextclass%> href="servlet/UserServlet?action=show&authority=1<%=next %>" aria-label="Next">
	...
	function setid(id,userName,userNumber,password){//修改模态窗口的id号
		changeid=id;
		document.changeUserForm.action = "servlet/UserServlet?action=change&authority=1&changeid="+changeid;
